...
describe('MemoryStorage', () => {
  ...
  describe('enqueue/dequeue', () => {
    ...
    it('should wait for job when queue is empty', async () => {
      const message = Buffer.from('delayed-msg')
  ...
  describe('workers', () => {
    ...
    it('should refresh worker TTL', async () => {
      await storage.registerWorker('worker-1', 100) // Short TTL
  ...
  describe('notifications', () => {
    ...
    it('should handle multiple subscribers', async () => {
      ...
      assert.deepStrictEqual(notifications.sort(), ['sub1:completed', 'sub2:completed'])
  ...
  describe('atomic operations', () => {
    it('should complete job atomically', async () => {
      ...
      let notified = false
      await storage.subscribeToJob('job-1', () => {
        notified = true
      })
    ...
    it('should retry job atomically', async () => {
      const message = Buffer.from('retry-test')

      await storage.enqueue('job-1', message, Date.now())
      await storage.dequeue('worker-1', 1)
      await storage.setJobState('job-1', 'processing:123:worker-1')

      await storage.retryJob('job-1', message, 'worker-1', 2)

      // Verify state
      const state = await storage.getJobState('job-1')
      assert.ok(state?.startsWith('failing:'))
      assert.ok(state?.endsWith(':2'))
